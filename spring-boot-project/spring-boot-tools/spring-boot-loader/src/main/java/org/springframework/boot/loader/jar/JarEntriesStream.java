...
class JarEntriesStream implements Closeable {
	...
	private boolean sameBytes(int len) {
		for (int i = 0; i < len; i++) {
			if (this.inBuffer[i] != this.compareBuffer[i]) {
				return false;
			}
		}
		return true;
	...
	private void fail(String check) {
		throw new IllegalStateException(String.format(
				"Content mismatch when reading security info for entry '%s' (%s check)", this.entry.getName(), check));
	}
